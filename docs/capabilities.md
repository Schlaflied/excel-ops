...
### 18. Local Agent MCP server

- **What it does:** exposes `excel_ops.scan_workdir`, `excel_ops.prepare_delivery`, `excel_ops.plan_delivery`, and `excel_ops.run_delivery` as discoverable MCP tools over local stdio, with versioned structured results and the existing Python CLI as the only execution backend.
- **Output and evidence:** MCP input/output schemas, tool annotations, structured environment/business failure categories, validated plan paths and digests, direct-CLI semantic parity tests, and an in-memory protocol test covering real `tools/list` and `tools/call` requests.
