...
A file changed after the apply is left exactly as it is and reported under `skipped` with `changed-after-apply`. Files added after the apply are never seen at all. Statuses are `rolled-back`, `rolled-back-partial` (something was skipped), `rolled-back-unverified` (the integrity check did not come back clean), `already-rolled-back`, `no-apply-recorded`, or `state-invalid`. A path that could not be written back, for example because it is locked, is reported under `skipped` with `restore-failed` instead of aborting the run; `refresh-manifest.json` itself follows the same path, so an unwritable manifest is skipped rather than throwing. Every restore write, the manifest included, goes through one helper that refuses a destination whose own path or any parent directory between the repository root and it is a symbolic link, and that writes through a temporary file renamed into place so the check cannot be raced. Such a destination is reported under `skipped` with `restore-failed` and a `detail` of `symlink-destination-rejected` or `symlink-parent-rejected`; nothing is written through the link, and the link is left untouched for you to inspect and remove before retrying. `.refresh/state.json` records the same status the run returned, so both `rolled-back-partial` and `rolled-back-unverified` stay open: running `rollback` again retries the skipped paths once their cause is resolved and reruns the integrity check. Entries already restored match the recorded hashes and are restored to the same bytes, so a retry is a no-op for them. Only a complete, verified rollback is recorded as `rolled-back` and reports `already-rolled-back` on a second run. After restoring, the integrity check reruns and its result is returned as `integrity`.
