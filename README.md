...
The same verified workbook can be delivered as XLSX, CSV, or PDF. CSV and PDF capability loss is explicit; PDF export can use LibreOffice or local Microsoft Excel and is checked for page-layout problems. See [Multi-format delivery](docs/multi-format-export.md).

Formula delivery is version-aware and verification-first: the agent can plan lookup, conditional-aggregation, or date formulas, apply formulas or static values to explicit ranges, and independently recalculate formula-mode output with Microsoft Excel or LibreOffice before delivery. See [Formula planning](docs/formula-planning.md).
...
**Done so far, all in Phase 1:** ingestion and layout detection ([#1](https://github.com/Schlaflied/excel-ops/issues/1)), type/locale inference ([#24](https://github.com/Schlaflied/excel-ops/issues/24)), schema-drift detection ([#10](https://github.com/Schlaflied/excel-ops/issues/10)), strict matching and dedup ([#2](https://github.com/Schlaflied/excel-ops/issues/2)), offline review packs ([#18](https://github.com/Schlaflied/excel-ops/issues/18)), safe template write-back ([#3](https://github.com/Schlaflied/excel-ops/issues/3)), independent delivery verification ([#4](https://github.com/Schlaflied/excel-ops/issues/4)), business-period resolution and date refresh ([#16](https://github.com/Schlaflied/excel-ops/issues/16), [#9](https://github.com/Schlaflied/excel-ops/issues/9)), safe output naming ([#17](https://github.com/Schlaflied/excel-ops/issues/17)), formula integrity checks ([#11](https://github.com/Schlaflied/excel-ops/issues/11)), batch ambiguity confirmation and Recipes ([#26](https://github.com/Schlaflied/excel-ops/issues/26)), working-directory scan ([#15](https://github.com/Schlaflied/excel-ops/issues/15)), idempotent execution ([#19](https://github.com/Schlaflied/excel-ops/issues/19)), delivery manifests ([#20](https://github.com/Schlaflied/excel-ops/issues/20)), semantic number/currency formats ([#23](https://github.com/Schlaflied/excel-ops/issues/23)), verified XLSX/CSV/PDF delivery ([#22](https://github.com/Schlaflied/excel-ops/issues/22)), natural-language formula planning, safe application, and independent recalculation ([#25](https://github.com/Schlaflied/excel-ops/issues/25)), the integrated `run_delivery(...)` pipeline ([#46](https://github.com/Schlaflied/excel-ops/issues/46)), cross-platform CI ([#50](https://github.com/Schlaflied/excel-ops/issues/50)), and a JS/TS Agent orchestration layer over MCP that forwards to the same Python CLI without reimplementing it ([#53](https://github.com/Schlaflied/excel-ops/issues/53)), plus agent-callable delivery-plan preparation ([#59](https://github.com/Schlaflied/excel-ops/issues/59)).
...
Every cloud connector is still out of scope. Working-directory scan results are still an explicit planning input rather than being selected automatically by `run_delivery`. Semantic currency and precision rules are documented in [the number-format policy guide](docs/number-format-policy.md).
